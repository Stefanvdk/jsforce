 ...
 * @returns {Subscription} - Faye subscription object
 */
Topic.prototype.subscribeWithReplay = function(
  replayFrom,
  extensions,
  listener
) {
  ...
  return this._streaming.subscribeWithExtension(
    this.name,
    allExtensions,
    listener
  );
...
Streaming.prototype._getFayeClient = function(channelName) {
  ...
  if (!this._fayeClients || !this._fayeClients[clientType]) {
    ...
    if (
      this._fayeClients[clientType]._dispatcher
        .getConnectionTypes()
 ...
 *
 * @param {String} name - Topic name
 * @param {Array} Extensions - Extensions to apply to subscription
 * @param {Callback.<Streaming~StreamingMessage>} listener - Streaming message listener
 * @returns {Subscription} - Faye subscription object
 */
Streaming.prototype.subscribeWithExtension = function(
  name,
  extensions,
  listener
) {
